...
public class ApiUtil {
    ...
    public static JSONObject get_article(HttpServletRequest request) {
        ...
        try{
            ...
            Vector<BeanArticleComment> articleComments=serviceArticleComment.get_comments(beanArticle.getArticle_id());
            if (articleComments==null){
                JSONObject data=new JSONObject();
                data.put("is_succeed","false");
                data.put("error_cause","article_comment_404");
                return data;
            }
            Vector<BeanRegisterUser> comment_users=new Vector<>();
            for (BeanArticleComment articleComment:articleComments){
                comment_users.add(serviceRegisterUser.get_user_by_id(articleComment.getArticle_comment_user_id()));
            }
            JSONObject data=new JSONObject();
            data.put("is_succeed","true");
            ...
            data.put("comments_count",articleComments.size());
            data.put("comments",articleComments.toArray());
            data.put("comments_user",comment_users.toArray());
            return data;
        }catch (Exception e){
            JSONObject data=new JSONObject();
            data.put("is_succeed","false");
    ...
    public static JSONObject do_article_like(HttpServletRequest request) {
        ServiceSession serviceSession;
        ServiceArticleLike serviceArticleLike;
        try {
            serviceSession=new ServiceSessionImpl();
            ...
            BeanSession beanSession=serviceSession.get_logged_user(request);
            String user_id=beanSession.getSession_user_id();
            String article_id=request.getParameter("article_id");
            if (serviceArticleLike.do_like(user_id,article_id)){
                JSONObject data=new JSONObject();
                data.put("is_succeed","true");
                return data;
            }else{
    ...
    public static JSONObject add_comment(HttpServletRequest request) {
        ServiceSession serviceSession;
        ServiceArticleComment serviceArticleComment;
        try {
            serviceSession=new ServiceSessionImpl();
            ...
            String article_id=request.getParameter("article_id");
            if (serviceArticleComment.do_comment(user_id,comment,article_id)){
                JSONObject data=new JSONObject();
                data.put("is_succeed","true");
                return data;
            }else{
