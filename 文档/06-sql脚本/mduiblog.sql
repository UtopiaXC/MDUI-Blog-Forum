...
CREATE TABLE `article_comment`  (
  ...
  `article_comment_content` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
...
-- ----------------------------
INSERT INTO `web_message` VALUES (1, 'title', 'MDUI Blog Forum');
INSERT INTO `web_message` VALUES (2, 'footer', 'Powered By <a target=\"_blank\" href=\"https://github.com/UtopiaXC/MDUI-Blog-Forum\">MDUI Blog Forum</a> Designed By <a target=\"_blank\" href=\"https://www.utopiaxc.cn/\">UtopiaXC</a>');
...
-- ----------------------------
-- Triggers structure for table article_comment
-- ----------------------------
DROP TRIGGER IF EXISTS `BeforeCommentInsert`;
delimiter ;;
CREATE TRIGGER `BeforeCommentInsert` BEFORE INSERT ON `article_comment` FOR EACH ROW BEGIN
SET NEW.article_comment_time=NOW();
END
;;
delimiter ;

-- ----------------------------
-- Triggers structure for table article_like
-- ----------------------------
DROP TRIGGER IF EXISTS `BeforeInsert`;
delimiter ;;
CREATE TRIGGER `BeforeInsert` BEFORE INSERT ON `article_like` FOR EACH ROW BEGIN
SET NEW.article_like_time=NOW();
END
;;
delimiter ;
